...
def _make_journal_entry_for_depreciation(
    ...
    depreciation_cost_center,
    depreciation_series,
    credit_account_1,  # First credit account
    debit_account_1,   # First debit account
    credit_account_2,  # Second credit account
    debit_account_2,   # Second debit account
    accounting_dimensions,
):
    # Check schedule range and journal entry posting conditions
    ...
    # First set of entries (credit and debit)
    credit_entry_1 = {
    ...
    # Second set of entries (credit and debit)
    credit_entry_2 = {
    ...
    # Apply accounting dimensions (e.g., cost center, department, etc.) to all entries
    for dimension in accounting_dimensions:
        if asset.get(dimension["fieldname"]) or dimension.get("mandatory_for_bs"):
            credit_entry_1.update(
                {
                    dimension["fieldname"]: asset.get(dimension["fieldname"])
                    or dimension.get("default_dimension")
                }
            )
            credit_entry_2.update(
                {
                    dimension["fieldname"]: asset.get(dimension["fieldname"])
                    or dimension.get("default_dimension")
                }
    ...
    # Append all four entries (two credit, two debit) to the Journal Entry
    je.append("accounts", credit_entry_1)
    je.append("accounts", debit_entry_1)
    je.append("accounts", credit_entry_2)
    je.append("accounts", debit_entry_2)

    # Ignore permissions and mark the entry as a planned depreciation entry
    je.flags.ignore_permissions = True
    je.flags.planned_depr_entry = True
    je.save()

    # Link the Journal Entry to the depreciation schedule
    depr_schedule.db_set("journal_entry", je.name)

    # Submit the Journal Entry if no workflow exists and update finance book depreciation values
...
@frappe.whitelist()
def get_gl_entries_on_asset_disposal_aq(
	asset, selling_amount=0, finance_book=None, voucher_type=None, voucher_no=None, date=None
):
	if not date:
		date = getdate()

	# Retrieve company-specific accounts
	company = frappe.get_doc("Company", asset.company)
	debit_account_designated_fund_account = company.custom_default_designated_asset_fund_account
	credit_account_income = company.custom_default_income
	print("Accountssss")
	print(debit_account_designated_fund_account)
	print(credit_account_income)
	...
	gl_entries = [
		# First entry: Credit the Fixed Asset Account
		asset.get_gl_dict(
			{
				"account": fixed_asset_account,
	...
	if accumulated_depr_amount:
		# Second entry: Debit the Accumulated Depreciation Account
		gl_entries.append(
			asset.get_gl_dict(
				{
					"account": accumulated_depr_account,
	...
	# Check if the additional accounts and value_after_depreciation are valid
	if debit_account_designated_fund_account and credit_account_income and value_after_depreciation > 0:
		frappe.msgprint(_("Adding two additional GL entries"))

		# Third entry: Debit the Designated Fund Account
		gl_entries.append(
			asset.get_gl_dict(
				{
					"account": debit_account_designated_fund_account,
		...
		# Fourth entry: Credit the Income Account
		gl_entries.append(
			asset.get_gl_dict(
				{
					"account": credit_account_income,
		...
		frappe.msgprint(_("Skipping additional GL entries due to missing or zero values"))
	print("profit_amount")
	print(profit_amount)
	...
	print(value_after_depreciation)
	# Handle profit or loss on asset disposal
	...
	# Attach voucher type and number if provided
	if voucher_type and voucher_no:
		for entry in gl_entries:
			entry["voucher_type"] = voucher_type
			entry["voucher_no"] = voucher_no
